...
def main(control_pub, ref_msg):
    ...
    if experiment_number == 1:
        print("Experimento: 1")
        T_ref[0, :]=  0.4*np.cos(1*t)+0.3*np.sin(0.4*t)
        T_ref[1, :] = 0.2*np.cos(t)*np.cos(0.5*t)+0.25*np.sin(0.7*t)
        T_ref[2, :] = 0.5*np.sin(t)*np.cos(0.5*t) 
        T_ref[3, :] = 0.5*np.sin(t)*np.sin(0.5*t)+0.3*np.cos(0.7*t)*np.cos(0.3*t)
    elif experiment_number == 2:
        print("Experimento: 2")
        T_ref[0, :]=  0.7*np.cos(0.5*t)+0.2*np.sin(0.4*t)
        T_ref[1, :] = 0.1*np.cos(1*t)+0.1*np.cos(2*t)
        T_ref[2, :] = 0.1*np.cos(2*t)+0.1*np.cos(5*t) 
        T_ref[3, :] = 1*np.sin(2*t)
    elif experiment_number == 3:
        print("Experimento: 3")
        T_ref[0, :]=  0.5*(0.7*np.cos(1*t)+0.5*np.sin(3*t))
        T_ref[1, :] = 0.6*(0.7*np.cos(2*t)+0.2*np.sin(0.4*t))
        T_ref[2, :] = 0.1*np.cos(2*t)+0.1*np.cos(5*t) 
        T_ref[3, :] = 1*np.sin(2*t) #(0.25 *np.cos(2*t)+ 0.25 *np.sin(0.4*t))
    elif experiment_number == 4:
        print("Experimento: 2")
        T_ref[0, :]=  0.7*np.cos(0.5*t)+0.2*np.sin(0.4*t)
        T_ref[1, :] = 0.1*np.cos(1*t)+0.1*np.cos(2*t)
        T_ref[2, :] = 0.1*np.cos(1*t)+0.1*np.cos(2*t) 
        T_ref[3, :] = 1*np.sin(2*t)
    elif experiment_number == 5:
        print("Experimento: 5")
        T_ref[0, :]=  0.7*np.cos(0.5*t)+0.2*np.sin(0.4*t)
        T_ref[1, :] = 0.3*np.cos(1*t)+0.1*np.cos(2*t)
        T_ref[2, :] = 0.3*np.cos(2*t)+0.1*np.cos(5*t) 
        T_ref[3, :] = 1*np.sin(1*t)
    elif experiment_number == 10:
        for i in range(4):
            signal = generate_signal(t, i)
            if i in [1, 2]:  # Aplicar suavizado solo a las señales 2 y 3
                T_ref[i, :] = smooth_signal(limit_amplitude(signal, 0.3))
            else:
                T_ref[i, :] = limit_amplitude(signal, 0.3)
